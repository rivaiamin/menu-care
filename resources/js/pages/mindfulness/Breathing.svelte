<script lang="ts">
    ...
    import { Wind, ArrowLeft, Loader2, Video, Headphones } from 'lucide-svelte';
    ...
    onMount(async () => {
        try {
            ...
            error = 'Gagal memuat konten relaksasi';
            loading = false;
            console.error('Error loading breathing content:', err);
        }
    });
...
<svelte:head>
    <title>Relaksasi Nafas Dalam - MeNu Care</title>
</svelte:head>
...
<AppLayout {breadcrumbs}>
    <div class="space-y-6 px-4 pt-4">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold tracking-tight">Relaksasi Nafas Dalam</h1>
                <p class="text-muted-foreground mt-2">
                    Video dan audio latihan pernapasan dalam untuk membantu relaksasi
                </p>
            </div>
            ...
            <Tabs bind:value={activeTab}>
                ...
                <TabsContent value="videos" class="mt-6">
                    {#if videos.length === 0}
                        <Card>
                            <CardContent class="pt-6">
                                <p class="text-muted-foreground">Belum ada video relaksasi yang tersedia untuk kategori Anda.</p>
                            </CardContent>
                        </Card>
                    {:else}
                ...
                <TabsContent value="audios" class="mt-6">
                    {#if audios.length === 0}
                        <Card>
                            <CardContent class="pt-6">
                                <p class="text-muted-foreground">Belum ada audio relaksasi yang tersedia untuk kategori Anda.</p>
                            </CardContent>
                        </Card>
                    {:else}
                        <div class="grid gap-6 md:grid-cols-2">
